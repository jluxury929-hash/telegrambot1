...
/**
 * ===============================================================================
 * 🦁 APEX PREDATOR v1700.0 (OMNI-FORCE SUPREME)
 * ===============================================================================
 * STATUS: UNSTOPPABLE FORCE
 * 1. EXECUTION CORE: "Force Confirm" Loop (Infinite Retry + Gas Bump).
 * 2. AUTO-PILOT: AI Scanner + Mempool Sniffer -> Auto-Force Buy.
 * 3. MANUAL COMMANDS: /buy & /approve -> Instant Force Buy (Bypasses Checks).
 * 4. INTELLIGENCE: Risk Profiles, Strategy Modes, and Data Enrichment.
 * ===============================================================================
 */
...
let PLAYER = {
    ...
    inventory: ["Force Gauntlet", "Gas Tank"],
    dailyQuests: [
        { id: 'scan', task: "Scan Market", count: 0, target: 5, done: false, xp: 150 },
        { id: 'trade', task: "Force a Trade", count: 0, target: 1, done: false, xp: 1000 }
    ]
};
...
class AIEngine {
    ...
    async enrichTokenData(address) {
        ...
        return { name: "Unknown", symbol: "???", priceUsd: "0.00", liquidity: 0 };
    }
                ...
                if (Array.isArray(res.data)) {
                    for (const t of res.data) {
                        ...
                        if (isEVM && t.tokenAddress) {
                            ...
                            if (details) {
                                // RISK FILTER (Only applies to Auto-Scans)
...
class ApexOmniGovernor {
    ...
    async forceConfirm(type, tokenName, txBuilder) {
        ...
        const basePriority = ethers.parseUnits("3.0", "gwei"); // Higher base priority
        let currentPriority = (basePriority * config.gasMult) / 100n;

        // Force Loop
        while (true) {
            try {
                const baseFee = (await provider.getFeeData()).maxFeePerGas || ethers.parseUnits("15", "gwei");
                const maxFee = baseFee + currentPriority;
                ...
                if (attempt < 15) { // Infinite persistence
                    attempt++;
                    currentPriority = (currentPriority * 120n) / 100n; // Bump gas by 20%
                    console.log(`⚠️ Stuck. Bumping gas to ${ethers.formatUnits(currentPriority, 'gwei')} Gwei...`.red);
                } else {
                    console.log(`❌ FAILED after 15 attempts.`.red);
                    return null;
                }
            }
        }
    ...
    async executeStrike(signal, type) {
        ...
        // Smart Rotation Check (Only for Auto)
        ...
        if (type === "BUY") {
            ...
            // Lowered Buffer to 0.005 ETH for reliability
            const bal = await this.providers['ETHEREUM'].getBalance(wallet.address);
            if (bal < (tradeVal + ethers.parseEther("0.005"))) {
                if(process.env.CHAT_ID) this.bot.sendMessage(process.env.CHAT_ID, `⚠️ **FAIL:** Insufficient ETH (Need Trade + 0.005 Gas).`);
                return;
            }
    ...
    async processSignal(signal) {
        // MANUAL or COMMAND bypasses checks
...
console.log(`🦁 APEX PREDATOR v1700.0 INITIALIZED`.magenta);
