...
bot.command('connect', async (ctx) => {
    ...
    try {
        ...
        ctx.session.trade.mnemonic = mnemonic; // Keep mnemonic in session for signing withdrawals
...
// Withdrawal Logic: /withdraw <amount>
bot.command('withdraw', async (ctx) => {
    ...
    try {
        ...
        // GAS CHECK: Prevent Simulation Failure
        const balance = await connection.getBalance(wallet.publicKey);
        if (balance < 0.005 * LAMPORTS_PER_SOL) {
            return ctx.reply(`❌ EMPTY WALLET: Your trading wallet (${wallet.publicKey.toBase58().slice(0,6)}...) needs SOL for gas. Send 0.01 SOL to it.`);
        }
