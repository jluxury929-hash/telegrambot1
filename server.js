...
// 2. Initialize Jito No-Auth Client (Public Portal 2026)
const jito = searcherClient('ny.mainnet.block-engine.jito.wtf'); 

// --- 🔮 CLEANED PYTH PUBLIC KEYS (Solana Mainnet) ---
const PYTH_ACCOUNTS = {
    'BTC/USD': new PublicKey("GVXRSBjTuSpgU9btXLYND1nKfCukS8VvfRmavRhvyr"),
...
// --- CAD Converter (Real-time 2026 rate: ~1.37) ---
async function getCADProfit(usd) {
    try {
        const res = await axios.get('https://api.exchangerate-api.com/v4/latest/USD');
        return (usd * res.data.rates.CAD).toFixed(2);
    } catch {
        return (usd * 1.37).toFixed(2); // Updated Feb 2026 Rate
    }
}
...
bot.action('exec_final', async (ctx) => {
    if (!ctx.session.trade.connected) return ctx.reply("❌ Connect wallet first with /connect");
...
bot.launch().then(() => console.log("🚀 Pocket Robot Integrated is Live!"));
