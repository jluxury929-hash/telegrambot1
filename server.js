...
async function executeRotation(chatId, targetToken, rawSymbol) {
    try {
        ...
        // Metadata Sanitizer for .png tickers
        ...
        startTrailingHarvest(chatId, { addr: targetToken, symbol, entry: res.data.outAmount });
...
bot.on('callback_query', async (q) => {
    ...
    if (q.data === "cmd_auto") {
        if (!solWallet) return bot.answerCallbackQuery(q.id, { text: "❌ Sync Wallet First!", show_alert: true });
